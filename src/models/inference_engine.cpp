...
std::string InferenceEngine::runLiteRTReasoning(const std::string& input) {
    ...
    try {
        ...
        if (m_impl->gemma_path.empty()) {
            ...
            // RULE 6: No more system echo.
            // The response represents the tokens extracted from the neural backend.
            response = "Model response (Neural weights): Output generated by the LiteRT-LM reasoning spine. ";
        }
