...
Node* GraphExecutor::selectNextNode(const std::string& input) {
    ...
    // --- STRICT HARDWARE BYPASS v3.9.4-STABLE ---
    ...
    if (hasVerb && hasObject) {
        LOGI(TAG, "> Route: Strict Bypass (Intent: SystemControl) [v3.9.4-STABLE]");
        return m_graph.getNode(4);
    }

    if (clean == "where am i" || clean == "get location" || clean == "gps status") {
        LOGI(TAG, "> Route: Strict Bypass (Intent: Location) [v3.9.4-STABLE]");
    ...
    if (isSearchPrefix || isFilename) {
        ...
        if (searchNode) {
            LOGI(TAG, "> Route: Neural Bypass (Intent: Search) [v3.9.4-STABLE]");
            return searchNode;
        }
    }
...
Node* GraphExecutor::runThompsonSampling(const std::string& input) {
    ...
    LOGI(TAG, "Reasoning Spine active: [Kernel v3.9.4-STABLE]");
