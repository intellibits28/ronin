        ...
        try {
            ...
            val rawResponseText = kotlinx.coroutines.withTimeout(timeoutMillis) {
                if (useLocalModel) {
                    ...
                    val text = resObj.optString("text", resObj.optString("response", "")).trim()
                    if (text.isBlank() || text.startsWith("Error:", true)) {
                        throw ShmError.AIReviewError("Cloud AI review failed: ${if (text.isBlank()) "No explanation generated by cloud model." else text}")
                    }
                    text
                }
            }
